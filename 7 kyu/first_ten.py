...
def filter_list(l):
    """Return list of int from list l"""
    return [i for i in l if isinstance(i, int)]


def is_square(n):
    """Return True if n is a square number of some int"""
    return n >= 0 and n ** 0.5 % 1 == 0


def accum2(s):
    """Return a little bit modified s"""
    return '-'.join([letter.upper() + letter.lower() * index for index, 
        letter in enumerate(s)])


def is_onumber_equal_xnumber(string):
    """Return True if the number of 'o' in the string is equal to the
    number of 'x'"""
...
def find_shortest_length(string):
    """Return a length of the shortest word(s)"""
    return min([len(word) for word in string.split()])


def maskify(string):
    """Return a string in which all symbols besides last 4 are
    masked"""
    return '#' * (len(string) - 4) +  string[-4:]


def sum_two_smallest_numbers(numbers):
    """Return sum of 2 smallest numbers in numbers"""
    numbers.sort()
